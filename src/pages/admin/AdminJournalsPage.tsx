...
import { adminApi, type AdminIssue, type AdminIssueDetail, type AdminJournal } from '../../lib/admin-api';
...
function IssueFormModal({
  init, initCoverUrl, journals, title, onSave, onClose,
}: {
  init:         typeof EMPTY;
  initCoverUrl: string | null;
  journals:     AdminJournal[];
  title:        string;
  onSave:       (d: Record<string, unknown>, cover: File | null) => Promise<void>;
  onClose:      () => void;
}) {
...
export default function AdminJournalsPage() {
  ...
  async function handleCreate(data: Record<string, unknown>, cover: File | null) {
    const iss = await adminApi.issues.create(data, cover);
    setIssues(p => [iss, ...p]); setCreating(false); flash('✓ Yangi jurnal soni yaratildi!');
  }

  async function handleUpdate(data: Record<string, unknown>, cover: File | null) {
    if (!editing) return;
    const upd = await adminApi.issues.update(editing.id, data as Partial<AdminIssue>, cover);
      ...
      <div style={{ padding: '40px var(--px) 28px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <button onClick={() => setCreating(true)}
            style={{ height: 44, padding: '0 20px', borderRadius: 8, border: 0, background: 'var(--navy)', color: 'white', cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--sans)', flexShrink: 0 }}>
            + Yangi son yaratish
          </button>
        </div>
